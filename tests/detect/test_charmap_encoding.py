...
import graphify.llm.core as llm_core
...
class TestSubprocessEncoding:
    ...
    def test_subprocess_called_with_utf8_encoding(self, monkeypatch):
        ...
        monkeypatch.setattr(llm_core, "_response_is_hollow", lambda raw, parsed: False)
        with patch("shutil.which", return_value="/fake/bin/claude"), \
             patch("subprocess.run", return_value=completed) as mock_run:
            llm.clients.claude._call_claude_cli(_UNICODE_CONTENT, max_tokens=8192)
