...
**Output directory:** All paths below use `$GOUT` instead of a hardcoded directory name. Before running any bash code block, prepend this line so the variable is available in that shell invocation:
```bash
...
```python
python -c "import graphify; import sys; from pathlib import Path; Path('$GOUT').mkdir(exist_ok=True); Path('$GOUT/.graphify_python').write_text(sys.executable)"
```
...
Path('$GOUT/.graphify_detect.json').write_text(json.dumps(result, indent=2))
...
detect = json.loads(Path('$GOUT/.graphify_detect.json').read_text())
code_files = []
...
if code_files:
    ...
    Path('$GOUT/.graphify_ast.json').write_text(json.dumps(result, indent=2))
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('$GOUT/.graphify_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}))
...
if cached_nodes or cached_edges:
    Path('$GOUT/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}))
Path('$GOUT/.graphify_uncached.txt').write_text('\n'.join(uncached))
...
ast = json.loads(Path('$GOUT/.graphify_ast.json').read_text())
all_nodes.extend(ast.get('nodes', []))
all_edges.extend(ast.get('edges', []))

cached_path = Path('$GOUT/.graphify_cached.json')
...
Path('$GOUT/.graphify_extract.json').write_text(json.dumps(merged, indent=2))
...
extraction = json.loads(Path('$GOUT/.graphify_extract.json').read_text())
...
Path('$GOUT/graph.json').write_text(json.dumps(graph_data, indent=2))
Path('$GOUT/.graphify_analysis.json').write_text(json.dumps({
...
extraction = json.loads(Path('$GOUT/.graphify_extract.json').read_text())
analysis = json.loads(Path('$GOUT/.graphify_analysis.json').read_text())
...
Path('$GOUT/GRAPH_REPORT.md').write_text(report)
...
try:
    to_html(G, communities, '$GOUT/graph.html')
...
Read `$GOUT/GRAPH_REPORT.md` and share the **God Nodes** and **Surprising Connections** sections directly in the chat — do not ask the user to open the file themselves.
