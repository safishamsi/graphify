...
import graphify.cli.main as mainmod
...
def test_extract_exits_nonzero_when_all_semantic_chunks_fail(
    ...
    monkeypatch.setattr(
        "graphify.llm.core.extract_corpus_parallel", _all_chunks_failed
    )
...
def test_extract_succeeds_when_at_least_one_chunk_completes(
    ...
    monkeypatch.setattr(
        "graphify.llm.core.extract_corpus_parallel", _one_chunk_succeeded
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
