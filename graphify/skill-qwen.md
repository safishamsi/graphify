...
**IMPORTANT - subagent type:** Always use the general-purpose agent type. Do NOT use read-only agents - they cannot write chunk files to disk, which silently drops extraction results. Use agents with Write and Bash access.
...
If more than half the chunks failed or are missing, stop and tell the user to re-run and ensure general-purpose agents are used.
...
G = build_from_json(json.loads(Path('graphify-out/.graphify_extract.json').read_text()))
to_graphml(G, 'graphify-out/graph.graphml')
print('graph.graphml written - open in Gephi, yEd, or Cytoscape')
"
```

### Step 8 - Wiki generation (only if --wiki flag)
...
print('Wiki written to graphify-out/wiki/ - index.md + one article per community')
"
```

### Step 9 - Print final summary + token benchmark
...
# Token benchmark
tokens = json.loads(Path('graphify-out/.graphify_extract.json').read_text())
total = tokens.get('input_tokens', 0) + tokens.get('output_tokens', 0)
print()
print(f'Tokens used: {total:,} (input: {tokens.get(\"input_tokens\", 0):,}, output: {tokens.get(\"output_tokens\", 0):,})')
print()
print('Support graphify: https://github.com/sponsors/safishamsi')
"
```

### Step 10 - Save Q&A results (if query/path/explain was run)

If the user ran `graphify query`, `graphify path`, or `graphify explain`, save the output:

```bash
graphify save-result "QUESTION" "ANSWER_TEXT"
```

This saves to `graphify-out/memory/` for future reference.

## File types graphify understands

| Type | Extensions | Extraction |
|------|-----------|------------|
| Code | `.py .ts .js .jsx .tsx .mjs .go .rs .java .c .cpp .rb .cs .kt .scala .php .swift .lua .zig .ps1 .ex .exs .m .mm .jl .vue .svelte .v .sv .dart` | AST via tree-sitter + call-graph (cross-file for all languages) + docstring/comment rationale |
| Docs | `.md .mdx .html .txt .rst` | Concepts + relationships + design rationale |
| Office | `.docx .xlsx` | Converted to markdown then extracted (requires `pip install graphifyy[office]`) |
| Papers | `.pdf` | Citation mining + concept extraction |
| Images | `.png .jpg .webp .gif` | Vision - screenshots, diagrams, any language |
| Video / Audio | `.mp4 .mov .mkv .webm .avi .m4v .mp3 .wav .m4a .ogg` | Transcribed locally with faster-whisper (requires `pip install graphifyy[video]`) |
| YouTube / URLs | any video URL | Audio downloaded via yt-dlp, then same Whisper pipeline (requires `pip install graphifyy[video]`) |

## After a graph is built

Tell the user:
1. **Open `graphify-out/graph.html`** in any browser - interactive graph, click nodes, search, filter by community
2. **Read `graphify-out/GRAPH_REPORT.md`** - god nodes, surprising connections, suggested questions
3. **Use `graphify query "<question>"`** to query the graph directly from the terminal
4. **Use `/graphify query "<question>"`** for BFS traversal, `/graphify query "<question>" --dfs` for DFS path tracing
