...
Run the shown `python -c "exec(...)"` commands directly in cmd.exe. Do not create temporary `.py` files. The Python payload uses escaped newlines and quotes so cmd.exe can paste it as one command.
