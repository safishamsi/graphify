...
def _devin_install_user(tmp_path):
    from graphify.installers.manager import install
    old_cwd = Path.cwd()
    try:
        import os
        os.chdir(tmp_path)
        with patch("graphify.installers.core.Path.home", return_value=tmp_path):
            install(platform_name="devin")
    finally:
        import os
        os.chdir(old_cwd)
...
def test_devin_install_project_creates_skill_file(tmp_path, monkeypatch):
    ...
    from graphify.cli.main import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["graphify", "devin", "install", "--project"])
    with patch("graphify.installers.core.Path.home", return_value=home):
        main()
...
def test_devin_rules_content_recommends_graphify_query(tmp_path):
    ...
    from graphify.installers.devin import _devin_rules_install
...
def test_devin_uninstall_user_removes_skill_file(tmp_path):
    ...
    from graphify.installers.core import _remove_skill_file
    with patch("graphify.installers.core.Path.home", return_value=tmp_path):
...
def test_devin_uninstall_user_noop_when_not_installed(tmp_path, capsys):
    ...
    try:
        ...
        os.chdir(old_cwd)
    out = capsys.readouterr().out
    assert "nothing to remove" in out or "not found" in out
...
def test_devin_rules_uninstall_noop_when_not_installed(tmp_path):
    ...
    from graphify.installers.devin import _devin_rules_uninstall
...
def test_devin_skill_file_exists_in_package():
    ...
    import graphify
    skill = Path(graphify.__file__).parent / "skills" / "skill-devin.md"
...
def test_devin_skill_file_uses_python_c_syntax():
    ...
    """
    import graphify
    skill = (Path(graphify.__file__).parent / "skills" / "skill-devin.md").read_text()
...
def test_devin_platform_skill_destination_user_scope(tmp_path):
    ...
    from graphify.installers.core import _platform_skill_destination
    with patch("graphify.installers.core.Path.home", return_value=tmp_path):
