...
def _mermaid_safe_label(s: str) -> str:
    """Escape a label for Mermaid flowchart syntax."""
    return sanitize_label(s).replace('"', "'").replace("\n", " ").replace("\r", " ")
...
def to_mermaid(
    ...
    """Export the graph as a Mermaid flowchart grouped by community."""
    node_community = _node_community_map(communities)
    node_aliases = {node_id: f"n{i}" for i, node_id in enumerate(G.nodes())}

    lines = [
        "flowchart LR",
        "%% Mermaid export - generated by graphify",
    ]

    for cid in sorted(communities):
        community_name = _mermaid_safe_label((community_labels or {}).get(cid, f"Community {cid}"))
        lines.append(f'  subgraph c{cid}["{community_name}"]')
        for node_id in sorted(communities[cid], key=lambda n: G.nodes[n].get("label", n).lower()):
            alias = node_aliases[node_id]
            label = _mermaid_safe_label(G.nodes[node_id].get("label", node_id))
            lines.append(f'    {alias}["{label}"]')
        lines.append("  end")

    covered = {node for nodes in communities.values() for node in nodes}
    uncategorized = [n for n in G.nodes() if n not in covered]
    if uncategorized:
        lines.append('  subgraph uncategorized["Uncategorized"]')
        for node_id in sorted(uncategorized, key=lambda n: G.nodes[n].get("label", n).lower()):
            alias = node_aliases[node_id]
            label = _mermaid_safe_label(G.nodes[node_id].get("label", node_id))
            lines.append(f'    {alias}["{label}"]')
        lines.append("  end")
    ...
    for u, v, data in G.edges(data=True):
        src = data.get("_src", u)
        tgt = data.get("_tgt", v)
        if src not in node_aliases or tgt not in node_aliases:
            continue
        relation = _mermaid_safe_label(data.get("relation", "related_to"))
        confidence = _mermaid_safe_label(data.get("confidence", "EXTRACTED"))
        edge_key = (src, tgt, relation, confidence)
        if edge_key in seen_edges:
            continue
        seen_edges.add(edge_key)
        lines.append(
            f'  {node_aliases[src]} -->|"{relation} [{confidence}]"| {node_aliases[tgt]}'
        )

    for cid in sorted(communities):
        color = COMMUNITY_COLORS[cid % len(COMMUNITY_COLORS)]
        lines.append(f"  style c{cid} fill:{color}22,stroke:{color},stroke-width:2px")
    if uncategorized:
        lines.append("  style uncategorized fill:#88888822,stroke:#888888,stroke-width:1px")
