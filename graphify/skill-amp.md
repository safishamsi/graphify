...
> **Amp platform:** Uses the `Task` tool to dispatch subagents. Multiple Task calls
> can be made in the SAME response for parallel execution. Each subagent has access
> to Read, Bash, edit_file, and create_file tools.

Call the Task tool once per chunk — ALL in the same response so they run in parallel.
If you make one Task call, wait, then make another, you are doing it sequentially and
defeating the purpose.

Concrete example for 3 chunks:
```
Task(prompt="...", description="Graphify extraction chunk 1/3")
Task(prompt="...", description="Graphify extraction chunk 2/3")
Task(prompt="...", description="Graphify extraction chunk 3/3")
```
All three in one message. Not three separate messages.

Each Task subagent receives this prompt (substitute FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, and DEEP_MODE):

```
You are a graphify extraction subagent. Your task:
1. Read each file listed below using the Read tool
2. Extract a knowledge graph fragment as JSON
3. Write the result to graphify-out/.graphify_chunk_CHUNK_NUM.json using create_file
...
Write the following JSON to `graphify-out/.graphify_chunk_CHUNK_NUM.json` using create_file (no other text):
...
After writing the JSON file, return a summary: how many nodes and edges were extracted.
```

**Step B3 - Collect, cache, and merge**

Wait for all Task subagents to return. For each result:
- Check that `graphify-out/.graphify_chunk_NN.json` exists on disk — this is the success signal
- If the file exists and contains valid JSON with `nodes` and `edges`, include it and save to cache
- If the file is missing, the subagent failed to write — print a warning: "chunk N missing from disk — subagent may have failed to write. Re-dispatch that chunk." Do not silently skip.
