...
- Feat: skill prompts now enforce a coverage guarantee — every `##` and `###` header in markdown files must produce at least one node with the section's source_location. Closes a gap where the LLM extractor silently skipped relevant sections (e.g., specific roadmap items in long backlog files).
